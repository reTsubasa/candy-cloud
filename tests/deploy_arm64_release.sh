...
for workflow in "$arm_workflow" "$x86_workflow"; do
	...
	grep -F 'RUNTIME_RELEASE_TAG: runtime-v0.4.0-r140' "$workflow" >/dev/null
	grep -F 'for attempt in $(seq 1 30); do' "$workflow" >/dev/null
	grep -F 'Runtime release $RUNTIME_RELEASE_TAG was not finalized within 150 seconds' "$workflow" >/dev/null
