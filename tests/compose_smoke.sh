...
printf '%s\n' "$compose_config" | awk '
  ...
  worker && /CORE_MODULE_VERSION: 0.3.51/ { version = 1 }
  worker && /CORE_MODULE_TARGET: x86_64-unknown-linux-gnu/ { architecture = 1 }
  worker && /CORE_MODULE_BUNDLE_URL:/ { url = 1 }
  worker && /CORE_MODULE_BUNDLE_SHA256: fcc4d5ac49cac071bbc1312195a0fbfe09ccb1609ebd08e09cbdfbb8826c77ff/ { bundle = 1 }
  worker && /CORE_MODULE_SHA256: c4b9dd9fb5bc05d3e2e4a6d79361a6d828b1bfaf6cd419b88cb0614418990f0c/ { module = 1 }
...
module_path=/opt/candy/cores/0.3.51/libcandy_core_cloud.so
test "$(docker exec "$worker_id" sha256sum "$module_path" | awk '{print $1}')" = \
  c4b9dd9fb5bc05d3e2e4a6d79361a6d828b1bfaf6cd419b88cb0614418990f0c
manifest_path=/opt/candy/cores/0.3.51/manifest.json
...
printf '%s\n' "$manifest" | grep -F '"commit":"9b67771dc492e189b5899ffafb37cfd51a8e6817"' >/dev/null
...
printf '%s\n' "$worker_logs" | grep -F '"module_version":"0.3.51"' >/dev/null
