...
async fn provision_initial_sdwan_trial(
    ...
    .await?;
    sqlx::query(
        "INSERT INTO entitlements (id, tenant_id, subscription_id, node_pool_id, service_permission, quota_json, status) VALUES (?, ?, ?, ?, 'private.tun.connect', JSON_OBJECT('allowed_features', 9217, 'max_outer_connections_per_node', 2, 'max_outer_connections_per_pool', 4, 'max_active_sessions_per_connection', 128, 'max_udp_flows_per_connection', 256, 'max_pending_opens', 32, 'max_speculative_streams', 8, 'max_datagram_record', ?, 'upload_rate_bps', 0, 'download_rate_bps', 0), 'ACTIVE')",
    )
    .bind(Uuid::now_v7())
    .bind(tenant_id)
    .bind(subscription_id)
