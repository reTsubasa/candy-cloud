...
node - "$repo_root" "$lua_syntax_file" "$status" "$diagnostics" <<'EOF'
...
assert.ok(source.includes("<%:Version%>"), "overview must show one product version row");
...
assert.match(rows[1].children[5].textContent, /CUBIC \(test\)/, "runtime BBR failover must show the effective CUBIC controller and reason");
assert.equal(rows[1].children[0].textContent, "sg");

context.refreshCandyDiagnosticsStatus();
requests[2].respond(200, { runtime: { performance: {} } });
...
for file in "$makefile" "$config" "$init" "$po" "$po2lmo_c" "$po2lmo_lmo" "$po2lmo_h" "$process_helper" "$controller" "$nodes" "$dns" "$advanced" "$rules" "$status" "$diagnostics" "$dns_tunnel_status" "$logs"; do
	assert_file "$file"
done
...
assert_contains "$makefile" '^PKG_VERSION:=0\.3\.4$'
...
for route in overview traffic dns_geo nodes diagnostics logs advanced; do
...
assert_contains "$advanced" 'process\.run\(\{ "/etc/init.d/candy", "reload_runtime" \}, \{ background = true \}\)'
...
assert_contains "$status" 'value\.authorized \? "inactive" : "unauthorized"'
...
assert_contains "$status" 'Fragment frame bytes'
assert_contains "$status" 'Early streams opened'
assert_contains "$status" 'Fallback transitions'
...
assert_contains "$status" '<%:Version%>'
...
assert_count "$diagnostics" '<div class="cbi-map">' 3
...
assert_contains "$diagnostics" 'fallback_reason'
