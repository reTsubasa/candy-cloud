...
grep -F 'status|running|enabled|stop|provider_update_once|provider_update_loop|network_apply|network_cleanup|reload_runtime|restart_queued|run_client|run_sdwan|run_netd)' "$repo_root/candy-client/candy.init" >/dev/null
...
grep -q '"version":"0.3.4"' "$NODE_STATUS_FILE"
! grep -q '"build_id"' "$NODE_STATUS_FILE"
grep -q '"runtime":{"mode":"fallback","performance":{"mode":"auto","lanes":"auto","udp_client_multiplier":2,"udp_server_multiplier":3}}' "$NODE_STATUS_FILE"
