...
|---|---|---|---|
| Core data plane `0.3.49` | Supported, musl | Supported, musleabihf | Supported, glibc |
| Core Cloud ABI `0.3.49` | Supported, glibc | Not applicable | Supported, glibc |
...
`0.3.49` artifacts. OpenWrt ARM64 remains intentionally unclaimed until a
...
1. `core-v0.3.49` contains three signed data-plane targets and two signed Cloud ABI targets.
2. `candy-release` verifies all five native targets and publishes both ARM64 records in the stable catalog.
3. Candy Cloud publishes `cloud-arm64-<commit>` from the signed `core-v0.3.49` ABI asset.
