...
|---|---|---|---|
| Core data plane `0.3.53` | Supported, musl | Supported, musleabihf | Supported, glibc |
| Core Cloud ABI `0.3.53` | Supported, glibc | Not applicable | Supported, glibc |
| OpenWrt Runtime `0.4.0-r129` | Supported, OpenWrt 25.12.4 x86/64 | Supported, IPQ40xx Cortex-A7 | Not released |
| Linux Runtime `0.4.0-r129` | Supported, musl | Not released | Supported, glibc |
...
`0.3.53` artifacts. OpenWrt ARM64 remains intentionally unclaimed until a
...
1. `core-v0.3.53` contains three signed data-plane targets and two signed Cloud ABI targets.
2. `candy-release` verifies all five native targets and publishes both ARM64 records in the stable catalog.
3. Candy Cloud publishes `cloud-arm64-<commit>` from the signed `core-v0.3.53` ABI asset.
