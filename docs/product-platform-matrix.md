...
## Source support matrix as of 2026-09-17

| Component | x86_64 / amd64 | ARMv7 / IPQ40xx | ARM64 / aarch64 |
|---|---|---|---|
| Core data plane `0.3.54` | Supported, musl | Supported, musleabihf | Supported, glibc |
| Core Cloud ABI `0.3.54` | Supported, glibc | Not applicable | Supported, glibc |
| OpenWrt Runtime `0.4.0-r133` | Supported, OpenWrt 25.12.4 x86/64 | Supported, IPQ40xx Cortex-A7 | Not released |
| Linux Runtime `0.4.0-r133` | Supported, musl | Not released | Supported, glibc |
...
`0.3.54` artifacts. OpenWrt ARM64 remains intentionally unclaimed until a
...
| Core data plane | `aarch64-unknown-linux-gnu` | Linux/glibc | Linux ARM64 server/edge nodes | Supported |
| Core Cloud ABI | `x86_64-unknown-linux-gnu` | Linux/glibc | Cloud amd64 images | Existing |
| Core Cloud ABI | `aarch64-unknown-linux-gnu` | Linux/glibc | Cloud ARM64 images | Supported |
...
1. `core-v0.3.54` contains three signed data-plane targets and two signed Cloud ABI targets.
2. `candy-release` verifies all five native targets and publishes both ARM64 records in the stable catalog.
3. Candy Cloud publishes `cloud-arm64-<commit>` from the signed `core-v0.3.54` ABI asset.
