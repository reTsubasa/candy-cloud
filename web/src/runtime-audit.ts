...
export function runtimeAuditEventDescription(
  ...
  if (action === 'RUNTIME_FAIL_OPEN_ENTERED') {
    return `${runtimeFailureDetail(metadata.error_code, counters, '旧版 Runtime 在撤销路由前未持久化原始故障码，本事件无法还原具体数据面故障；升级后每次降级都会保留明确错误码')}；系统已撤销 SD-WAN 路由，未匹配流量继续按节点本地网络策略转发。`;
  }
