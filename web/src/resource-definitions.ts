...
export const resourceDefinitions: ResourceDefinition[] = [
  ...
  { key: 'segments', label: '网络分段', collection: 'segments', kind: 'SEGMENT', description: '共享路由与策略的站点网络；不同分段默认隔离', emptyTitle: '尚未创建网络分段' },
...
export function defaultSpec(kind: string): ResourceSpec {
  const examples: Record<string, Record<string, unknown>> = {
    ...
    SERVICE_POLICY: { name: '', segment_id: '', generation: 1, enabled: true, rules: [] },
