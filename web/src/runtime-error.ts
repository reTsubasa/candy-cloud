...
const runtimeErrors: Record<string, RuntimeErrorDescriptor> = {
  ...
  peer_prepare_failed: core('候选链路准备失败', '该 Peer 的候选认证或隧道准备失败，或准备任务异常退出；其他 Peer 的准备独立进行，请查看对应节点的失败详情'),
  peer_prepare_timeout: core('候选链路准备超时', '该 Peer 在本轮准备预算内未完成认证和数据流协商；请检查端点可达性、授权和备用端点的尝试结果'),
  peer_preparation_pending: core('等待候选出口就绪', '新策略仍有出站路由缺少已准备好的出口；Runtime 保留当前本地转发规则并稍后重试本次发布'),
  ...
  peer_stream_closed: core('对端 Stream 已关闭', '对端 QUIC 数据流已关闭，Runtime 将保留故障原因并重新协商线路'),
  peer_stream_read_failed: core('对端 Stream 读取失败', 'Candy Core 无法从对端 QUIC 数据流读取数据'),
  peer_stream_write_failed: core('对端 Stream 写入失败', 'Candy Core 无法向对端 QUIC 数据流发送数据'),
  ...
  core_policy_prepare_failed: core('候选策略准备失败', '新策略的候选链路准备失败、超时或发布内容已变化；Runtime 未切换本地流量规则，请结合节点日志查看具体原因'),
  core_policy_commit_unresolved: core('策略提交结果待确认', 'Runtime 已发送 Core 提交请求，但未能确认结果；当前保持降级转发并核对目标配置，不会盲目回退旧版本'),
  netd_lease_renewal_failed: core('网络规则续租失败', '策略切换期间无法续租 netd 网络规则；Runtime 保持降级并重试，请检查节点上的 netd 进程和通信日志'),
  ...
  netd_route_reconcile_failed: network('路由自愈提交失败', 'Runtime 已计算出目标路由集合，但 netd 未能把该集合提交到操作系统；节点会保留故障证据并继续重试'),
  route_snapshot_mismatch: network('路由快照不一致', '节点内核中的 Candy 路由与当前签名声明不一致，可能存在缺失、属性不匹配或未释放的孤儿路由'),
  route_snapshot_inspection_failed: network('路由快照读取失败', 'Runtime 无法完整读取操作系统中的 Candy 路由，不能确认声明路由是否已正确生效'),
  peer_stream_task_failed: core('SD-WAN 数据流任务异常退出', '该 Peer 的数据流任务发生 panic、取消或发送队列关闭；Core 关闭故障连接以触发重新协商，其他 Peer 保持运行'),
